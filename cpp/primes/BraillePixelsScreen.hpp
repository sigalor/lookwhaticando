...
class BraillePixelsScreen																							//idea from https://github.com/asciimoo/drawille
{
	private:
		...
		int hiBit(unsigned int n)																					//principle from http://stackoverflow.com/a/53175/3554605
		{
			int ret = 0;
			while(n >>= 1  &&  n)
				ret++;
	...
	public:
		#ifdef __unix__																								//more preprocessor operating system dependent defines at http://stackoverflow.com/a/8249232/3554605
		BraillePixelsScreen()
		{
			struct winsize w;																						//from http://stackoverflow.com/a/1022961/3554605
		...
		BraillePixelsScreen(unsigned int newWidth, unsigned int newHeight) : width(newWidth), height(newHeight)
		{
			...
			for(unsigned int y=0; y<height; y+=4)
			{
				for(unsigned int x=0; x<width; x+=2)
				{
					...
					else
					{
						...
						braille += 0x2800;																			//this is where the Braille block begins
		...
		void setChar(unsigned int x, unsigned int y, char c)														//x has to be in interval [0; width/2[, y in [0; height/4[
		{
		...
		void drawLine(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, bool set=true)			//Bresenham algorithm, slighly optimized to prevent line gaps
		{
			...
			for(unsigned int l=0; l<(1 << density); l++)
			{
