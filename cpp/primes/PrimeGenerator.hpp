...
{
	long integerSqrt(long num)																						//this is a nice method to get the integer square root of 'num' if it has one, from http://forum.nasm.us/index.php?topic=1694.0
	...
	void checkPrime(long number, long start, long end)
	{
		for(long i=start; i<=end; i++)
		{
			if(i%100000 == 0  &&  isPrime == false)
				break;
			if(number%i == 0)
			{
				isPrime = false;
	...
	std::vector<long> findPrimes(long start, std::size_t num, long* lastCurrLimit, long end=-1)						//when lastCurrLimit!=nullptr, then start has to be the last prime of the previous call of this function; end will not be included if it's a prime
	{
		long i, j;
		long currLimit;
		bool isPrime;
		...
		while(ret.size() < num)																						//when the function 'find_primes' the array 'buffer' will keep exactly 'size' primes, starting from (and possibly including) 'start'. You cannot define an ending prime in this function
		{
			...
			/*isPrime = true;
			checkPrimeStepWidth = 1 + ((currLimit-2+1 - 1) / numThreads);											//fast ceiling of integer division, from http://stackoverflow.com/a/2745086/3554605
